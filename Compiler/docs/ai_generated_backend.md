...
- 常量折叠：二元运算左右操作数均为常量时，编译期直接求值并生成 `li`。
- 常量传播：对已折叠出的常量结果进行缓存，后续使用时直接装载立即数。
- 局部公共子表达式消除（CSE）：同一基本块内相同二元运算只计算一次，后续复用结果。
- 简单死代码删除（DCE）：`used_by` 为空的 SSA 值不分配栈槽，也不生成计算指令。
- 条件分支简化：条件值为编译期常量时，直接生成 `j` 到目标块，省略比较与分支指令。
